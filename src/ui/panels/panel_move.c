...
static void on_jog(lv_event_t *e)
{
    ...
    mock_jog(axis, dir * dist_table[dist_idx]);
}
...
static void update_pos(void)
{
    ...
    for (int a = 0; a < 3; a++) {
        if (mock_homed(a)) {
            theme_fmt_float(vbuf, sizeof(vbuf), mock_pos(a), 1);
...
static lv_obj_t *create(void)
{
    ...
    for (int i = 0; i < 3; i++) {
        ...
        lv_obj_set_size(b, 96, 32);
        lv_obj_align(b, LV_ALIGN_BOTTOM_LEFT, 8 + i * 104, -8);
